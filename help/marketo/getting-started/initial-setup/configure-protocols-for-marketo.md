...
Marketo Engageが割り当てたデフォルトのトラッキングリンクである[MktoTrackingLink]を`[YourEmailCNAME]`が次の形式で示すように、送信されたメール CNAME マーケティングを追加します。
...
ベストプラクティスとして、DMARCの潜在的な影響を理解し、SPFとDKIMの連携を緩和するようにDMARC ポリシーを設定して、DMARC ポリシーをp=noneからp=quarantineに、p=rejectにエスカレーションして、DMARCの実装を徐々に展開することをお勧めします。
...
>[!CAUTION]
>
>このポリシーを慎重に使用し、自社に適しているかどうかを判断してください。
...
<tbody>
  ...
  <tr>
    ...
    <td>0：すべてが失敗した場合のレポートの生成
    <br>1：何かが失敗した場合のレポートの生成
    <br>d: DKIMが失敗した場合のレポートの生成
    <br>s: SPFが失敗した場合のレポートの生成</td>
...
>[!NOTE]
>
>Marketo Engageでは、DKIMとSPFでDMARCの整列を行うことをお勧めします。
...
MX レコードを使用すると、メールを送信するドメインにメールを送信して、返信と自動応答を処理できます。 会社のドメインから送信する場合は、既に設定されている可能性があります。 そうでない場合は、通常、会社ドメインの MX レコードにマッピングするように設定できます。
