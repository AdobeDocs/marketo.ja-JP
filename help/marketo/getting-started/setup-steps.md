...
>[!NOTE]
>
>会社が&#x200B;**Marketo**&#x200B;の新しいユーザーである場合にのみ、これらの手順を実行する必要があります。 そうでない場合は、既に設定が完了している可能性があります。
...
## ログインして、追加のマーケティングユーザーを作成{#log-in-and-create-additional-marketo-users}

1. 電子メールで受け取った資格情報を使用して、Marketor [ここ](https://app.marketo.com/)にログインします。

   ![](assets/new-login-screen-hand.jpg)

おめでとう！ 現在はMarketoにいて、開始が調査できます。 マーケティングチームの同僚を招待して参加させることもできます。 これは、新しいユーザーを追加することで行うことができます。
...
お客様は、会社のマーケティング担当者であることを伝える電子メールをMarketo Supportから受け取った可能性があります。 その場合は、**承認されたサポートの連絡先**&#x200B;をチーム用に設定できます。 認定されたサポート担当者のみが、[マーケットサポートポータル](https://support.marketo.com)を介して直接マーケティング担当者に連絡できます。

>[!NOTE]
>
>作成できるサポートの連絡先の数は、購入したパッケージによって異なります。 この制限は、マーケティングサポートからの電子メールで指定します。

承認されたサポートの連絡先ドキュメントは、マーケティングコミュニティに移動されました。 [この記事](https://nation.marketo.com/t5/Knowledgebase/Managing-Authorized-Support-Contacts/ta-p/254341)を参照してください。
...
電子メールができるだけ多くの人に届くように、いくつかの方法が考えられます。
    ...
    1.**トラッキングリンクをブランド化** Marketoからの電子メールに含めるリンクに、（マーケティング担当者の代わりに）独自のドメインを使用するCNAMEを選択できます。 これにより、ドメインのブランディングが強化され、受信者の信頼と配信品質が向上します。
    1.**追加企業の電子メール許可リストにマーケティング**テスト用の電子メールを実際のユーザーに送信する前に、テスト用の電子メールをテストアカウントに送信するのは、一般的なベストプラクティスです。 マーケティングを許可すると、そのようなテスト電子メールがブロックされたりスパムとしてフラグ付けされたりするのを防ぐことができます。
    1.**SPFとDKIMを設定します。**これらのテクノロジーにより、受信者はMarketorの電子メールがスパムでないことを保証します。 受信者のスパムフィルターがMarketoの電子メールを拒否するのを防ぐには、次の手順に従って[Setup a SPF and DKIM for Your Email Deliverability](/help/marketo/product-docs/email-marketing/deliverability/set-up-spf-and-dkim-for-your-email-deliverability.md)を実行します。
...
Marketotoトラッキングのリンクを探すには、**管理者**&#x200B;の領域に移動します。
...
`3)` 許可リストマーケティング。
    ...
    *電子メール許可リストでIPアドレスを使用する場合は、次のIPを追加します。
    199.15.212.0/22
...
`[`「 **From** Domain」をドメインからの電子メールに置き換えます(例：会社.com)と **** CorpIP（会社の電子メールサーバーのIPアドレス）255.255.255.255)。複数のドメインからMarketo経由で電子メールを送信する場合は、ITスタッフに各ドメインに対してこの行を（1行で）追加してもらう必要があります。`]`
...
## ITが{#complete-your-marketo-setup-after-it-finishes}を終了した後に、マーケティングのセットアップを完了する
...
会社が使用するCRMに応じて、次の中から選択します。
    ...
    * [MarketoをSalesforce.comと統合](/help/marketo/product-docs/crm-sync/salesforce-sync/understanding-the-salesforce-sync.md)
    * [MarkettoをMicrosoft Dynamicsと統合](/help/marketo/product-docs/crm-sync/microsoft-dynamics-sync/understanding-the-microsoft-dynamics-sync.md)
