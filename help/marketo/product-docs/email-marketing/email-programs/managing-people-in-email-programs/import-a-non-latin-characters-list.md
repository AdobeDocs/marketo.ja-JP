...
1. Excelの[フ **ァイル** ]メニューから[名前を付けて **保存** ]を選択します。

   ![](assets/image2015-2-10-9-3a46-3a44.png)

1. 「 **形式** 」オプションで、「 **UTF-16 Unicodeテキスト(.txt)** 」を選択します。 これにより、Marketorがファイルを表示できる方法でファイルがエンコードされます。
...
1. 新しいファイルは、.txt拡張子を持つテキストファイルとして保存されます。 また、ファイル内のすべてのカンマがタブに変換されます。 変えなきゃ。
   ...
   >[!TIP]
   >
   >Windowsを使用している場合は **メモ帳を使用してテキストファイルを開くことができ** 、Macを使用している場合は **TextEdit** を使用できます。
...
1. 手順4でコピーしたタブを最初の（置き換える）ボックスに貼り付け、2番目の（置き換える）ボックスにカンマを入力します。 「す **べて**」をクリックします。
