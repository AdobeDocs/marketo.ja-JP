...
product: marketo
title: フラグメント
description: フラグメントを再利用可能なメールコンポーネントとして作成および使用する方法について説明します。 メールおよびテンプレートにビジュアルコンテンツフラグメントを追加します。
...
>[!NOTE]
>
>_HTMLと同期_ がメールで無効になっている場合、加えられた変更（継承の解除を含む）はテキストコンテンツには反映されません。
