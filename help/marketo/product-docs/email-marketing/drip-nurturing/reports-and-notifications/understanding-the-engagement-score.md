...
description: エンゲージメントスコア（0～100）と、コンテンツの有効性の測定方法について説明します。 ダッシュボードを使用して、コンテンツのパフォーマンスを比較します。
...
スコアは、関与した行動（[!UICONTROL 開封]、[!UICONTROL クリック]、[!UICONTROL プログラムの成功]）と離脱した行動（[!UICONTROL 登録解除]）を考慮した独自のアルゴリズムに基づいています。ドリップとナーチャリングのスタイルのメールに対してベンチマークをおこない、平均 50 を達成しました。訪問者にコンテンツに関与する機会を与えるために、エンゲージメントスコアは各キャストの 72 時間後に計算されます。また、スコアは&#x200B;**最後の 3**&#x200B;キャストからのデータのみをカバーします。
