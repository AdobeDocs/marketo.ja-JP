...
1. 書き出し後、リストは txt ファイルとしてブラウザーのダウンロードフォルダーに表示されます。 取得し、静的リストとして Marketo インスタンスに[インポート](/help/marketo/getting-started/quick-wins/import-a-list-of-people.md)します。
   ...
   >[!CAUTION]
   >
   >これらのインボックスへの配置キャンペーンの数は 1 か月あたりで制限されています。 数を確認するには、[!DNL Everest] の[!UICONTROL アカウント設定]／[!UICONTROL サブスクリプション]の下の「[!UICONTROL サブスクリプション]」セクションを参照してください。 詳細については、Marketoの担当者にお問い合わせください。
