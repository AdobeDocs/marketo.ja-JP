...
description: SAML 2.0 シングルサインオンを有効にして、ID プロバイダーがMarketoのユーザーを認証します。
...
ユーザーを認証するディレクトリサービスをご利用の場合は、Marketo へのシングルサインオン（SSO）を許可することができます。 Marketoでは、[!DNL Security Assertion Markup Language] （SAML）バージョン 2.0以降を使用してこの機能をサポートしています。

Marketo は SAML サービスプロバイダー（SP）として機能し、ユーザーの認証には外部 ID プロバイダー（IdP）を使用します。

SSO を有効にすると、IdP がユーザーの資格情報を検証できるようになります。 ユーザーが Marketo ソフトウェアを使おうとすると、SP として機能する Marketo に対して IdP が署名済み SAML メッセージを送信します。 このメッセージは、お客様がMarketo ソフトウェアの使用を許可されていることをMarketoに確認します。
...
>[!IMPORTANT]
>
>これは、**not**&#x200B;がAdobe IDにオンボーディングされたサブスクリプションに適用されます。 以前の設定が（IMS移行前から）表示される場合がありますが、この設定は適用されません。 Adobe IDにオンボーディングされたサブスクリプションの場合、[&#x200B; シングルサインオンはAdobe Admin ConsoleのAdobe組織レベルで](https://helpx.adobe.com/jp/enterprise/using/set-up-identity.html){target="_blank"}設定されます。

>[!NOTE]
>
>[!DNL Microsoft Azure] ユーザの場合 [統合チュートリアル](https://learn.microsoft.com/ja-jp/entra/identity/saas-apps/marketo-tutorial){target="_blank"}をご覧ください。 チュートリアルの手順5cにタイプミスがあることに注意してください。 リレーの状態を`https://<munchkinid>.mktoweb.com`、**_ではなく_** `https://<munchkinid>.marketo.com`に設定します。
...
* **自動ユーザープロパゲーション** - 「ジャストインタイムプロビジョニング」とも呼ばれ、ユーザーの最初のSAML ログインで、アクセスしているweb アプリケーション（Marketoなど）でユーザーを作成でき、手作業による管理アクションが必要ない場合に発生します。 現時点では、Marketo ではサポートされていません。
