...
* **移行後は、URL の外観が異なります。**。Marketo Engage は、experience.adobe.com から Adobe Experience Cloud に提供される状態から移行し、URL は次の形式になります。`https://experience.adobe.com/#/@tenantID/so:XXX-XXX-XXX/marketo-engage/classic/`（XXX は Munchkin ID を表し、@tenantID は Adobe 組織からのアドレスです）。Marketo Engage へのアクセスが中断されないようにするには、IT チームと協力して、[この記事の上部](/help/marketo/getting-started/initial-setup/configure-protocols-for-marketo.md){target="_blank"}に記載されているすべての Adobe ドメインを許可リストに登録する必要があります。
...
* Marketo Engage アプリケーション自体では、**それ以外のすべての機能の管理方法に変更はありません**（ユーザロール、ワークスペース、動作の管理など）。ローカル（API のみ）のユーザ管理は、Marketo 管理者エリアの「_ユーザ＆ロール_」タブで引き続き行われます。
...
**Adobe は、最初に（標準の管理者ロールを持つ） Marketo 管理者を自動的に移行します**。Marketo 管理者が Admin Console 製品管理者ロールで Adobe ID に移行されると、以前に割り当てられていた他のロールで、Marketo アプリケーション内のアドビ製品管理者ロールが割り当てられます。

**Marketo サブスクリプションが Marketo やアドビ組織で SSO を使用していない場合**、アドビは残りのユーザを自動的に移行します。このワークフローは、最高レベルの自動化を実現して、Adobe Marketo ユーザのオーバーヘッドを最小限に抑えることを目的としています。移行を実行するためにユーザ側で必要なアクションはありません。

**Marketo サブスクリプションが Marketo やアドビ組織で SSO を使用している場合**、Marketo 管理者は、Marketo 管理者エリアにある Marketo Migration Console のセルフサービスユーザ移行エリアにアクセスできるようになります。ユーザ移行プロセス中により詳細な制御を必要とする場合、Marketo 管理者はいつかのバッチで移行するか、一度にまとめてすべてを移行するようユーザの選択を開始できます。ユーザを選択すると、管理者は「今すぐ移行」または後日の「移行をスケジュール」を選択できるので、どのユーザをいつ移行するかを非常に柔軟に制御できます。
