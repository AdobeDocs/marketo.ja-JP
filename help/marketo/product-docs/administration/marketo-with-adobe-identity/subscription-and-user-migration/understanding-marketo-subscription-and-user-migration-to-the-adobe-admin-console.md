...
>[!TIP]
>
>[Enterprise and Teams 管理ガイド ](https://helpx.adobe.com/jp/enterprise/admin-guide.html){target="_blank"} で、Adobe Admin Consoleを使用して組織全体でAdobeの使用権限を管理する方法を説明します。
...
* **移行後は、URL の外観が異なります。**。Marketo Engageは、experience.adobe.comからAdobe Experience Cloudに提供される状態から移行し、URL は次の形式になります。`https://experience.adobe.com/#/@tenantID/so:XXX-XXX-XXX/marketo-engage/classic/` （XXX はMunchkin ID を表し、@tenantID はAdobe組織からのアドレスです）。 Marketo Engage へのアクセスが中断されないようにするには、IT チームと協力して、[この記事の上部](/help/marketo/getting-started/initial-setup/configure-protocols-for-marketo.md){target="_blank"}に記載されているすべての Adobe ドメインを許可リストに登録する必要があります。
...
予定されていませんが、今後の開発作業でこのリダイレクト機能が機能しなくなる可能性があります。 予期しない中断を避けるために、できるだけ早い時期にブックマークを更新することをお勧めします。

## 変更されていない点 {#what-is-not-changing}

* **Marketo Engage アプリケーション自体の中で** 機能、ユーザーロール、ワークスペース、機能、およびビヘイビアーの管理を含む、その他すべての機能の管理方法に変更はありません。 ローカル（API のみ）のユーザー管理は、Marketo管理領域の「_ユーザーと役割_」タブに残ります。
...
>[!NOTE]
>
>現在 SSO を使用していないが実装を検討している場合は、ユーザ移行を行う前に実装することをお勧めします。シングルサインオンを実装する場合で、Adobe組織に SSO が実装されていないサブスクリプションがAdobe ID にオンボーディングされた場合は、[Marketo サポートにチケットを送信し ](https://nation.marketo.com/){target="_blank"} 「Admin ConsoleでのMarketo、SSO の実装」と指定します。
...
**Adobeでは、最初にMarketo管理者（標準の管理者ロールを持つ）が自動的に移行され** す。 Marketo管理者がAdmin Console製品管理者ロールを持つAdobe ID に移行されると、以前に割り当てられていた他のロールと共に、Marketo アプリケーション内のAdobe製品管理者ロールが割り当てられます。

**Marketo サブスクリプションがMarketoやAdobe組織に SSO がない場合**、Adobeは残りのユーザーを自動的に移行します。 このワークフローは、最高レベルの自動化を実現して、Adobe Marketo ユーザのオーバーヘッドを最小限に抑えることを目的としています。移行を実行するためにユーザ側で必要なアクションはありません。

**Marketo サブスクリプションがMarketoまたはAdobe組織（あるいはその両方）に SSO がある場合**、Marketo管理者は、Marketo管理領域にあるMarketo移行コンソールのセルフサービスユーザー移行エリアにアクセスできます。 ユーザーの移行プロセス中により詳細な制御を必要とするユーザーの場合、Marketo管理者は、バッチまたは一度にすべてを移行するユーザーの選択を開始できます。 ユーザを選択すると、管理者は「今すぐ移行」または後日の「移行をスケジュール」を選択できるので、どのユーザをいつ移行するかを非常に柔軟に制御できます。
...
>* [Adobe Identity Managementへの移行チュートリアル ](https://experienceleague.adobe.com/ja/docs/marketo-learn/tutorials/fundamentals/migrating-to-adobe-identity-management){target="_blank"}
