...
description: CRMまたはMarketoから潜在的なターゲットアカウントを見つける方法について説明します。 選択したアカウントを名前付きアカウントとして追加し、自動的にユーザーを関連付けます。
title: 顧客の選択
...
1. 新しいウィンドウ／タブが開きます。 [!UICONTROL 重点顧客]に追加する会社を選択し、「**[!UICONTROL 次へ]**」をクリックします。
   ...
   >
   >* Marketo データベースから、その会社がレコードに登録されているリードを検索します。 一部の属性（業種など）に複数の値が表示される場合は、Marketoが個々のユーザーに対して異なる値を見つけたためです。 ヒット数が最も多い属性が優先されます
   >
...
>[!NOTE]
>
>選択したCRM アカウントと「CRMを見つける」グリッドの内容に不一致が見られる場合は、次のいずれかまたは複数が原因である可能性があります。
>
