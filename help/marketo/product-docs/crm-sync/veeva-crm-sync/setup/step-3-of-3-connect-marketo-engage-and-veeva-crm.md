...
1. Marketo で、「**[!UICONTROL 管理者]**」をクリックします。 「**[!UICONTROL CRM]**」を選択し、「**[!UICONTROL Veeva と同期]**」をクリックします。
   ...
   >[!NOTE]
   >
   >[同期ユーザーからMarketoで必要でない](/help/marketo/product-docs/crm-sync/salesforce-sync/sfdc-sync-details/hide-a-salesforce-field-from-the-marketo-sync.md){target="_blank"}すべてのフィールドを非表示にしてから、「フィールドを同期」をクリックします。 「同期フィールド」をクリックすると、ユーザが表示できるすべてのフィールドが Marketo に作成され、削除できなくなります。
...
1. 数分後、Marketo Engageにポップアップが表示されます。 「**[!UICONTROL 資格情報を確認]**」をクリックします。
