---
description: OAuth 2.0を使用したログイン —Marketoドキュメント — 製品ドキュメント
title: OAuth 2.0を使用したログイン
...
# OAuth 2.0を使用したログイン{#log-in-using-oauth-2-0}

SalesforceはOAuthプロトコルを使用して、アプリケーションのユーザーが、ログイン資格情報を明らかにすることなく、安全にデータにアクセス（OAuth 2.0を使用してアプリケーションを認証）できるようにします。 以下は、MarketoとSalesforceを安全に接続し、同期するために実行する手順です。
...
>[!CAUTION]
>
>[**フィールドを同期**]をクリックする前に、Marketoで不要なフィールドを同期ユーザーからすべて非表示にしてください。 [フィールドの同期]をクリックすると、SFDCでユーザに表示されるすべてのフィールドがMarketoに永久的に作成され、削除できなくなります。
...
1. MarketoのSalesforceの同期ページで、「**Login with Salesforce**」ボタンをクリックします。
   ...
   >[!CAUTION]
   >
   >[ユーザ名]、[パスワード]、[トークン]の各フィールドが表示され、[Salesforceにログイン]ボタンが表示されない場合は、Marketo購読での基本認証が有効になります。 [基本認証を使用したMarketoのセットアップ](/help/marketo/product-docs/crm-sync/salesforce-sync/setup/enterprise-unlimited-edition/step-3-of-3-connect-marketo-and-salesforce-enterprise-unlimited.md)を参照してください。 一連の資格情報を使用して同期が開始されると、Salesforceの資格情報や購読を切り替えることはできません。 Oauth 2.0を使用する場合は、カスタマーサクセスマネージャーにお問い合わせください。
