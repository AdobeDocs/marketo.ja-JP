...
同期ステップは、オブジェクトスキーマおよびデータ自体に対する各オブジェクトタイプによるプッシュまたはプルの操作を反映します。 統計は、同期中の新しいレコード、更新、削除、失敗したカウントをカバーします。 日付、操作のタイプまたはオブジェクトのタイプでフィルタリングできます。 同期ステータスダッシュボードには、過去 5 日間の同期サイクルのステータスが表示されます。
...
1. の下 [!UICONTROL 統合]をクリックし、 **Salesforce**&#x200B;を、 **[!UICONTROL 同期ステータス]** タブをクリックします。

   ![](assets/salesforce-sync-status-2.png)

デフォルトでは、統計は最近開始された順に並べ替えられます。並べ替えアイコンをクリックして、「開始日」または「終了日」（最も新しい順）で並べ替えることができます。
...
1. 日付と時間の範囲を選択し、ドロップダウンをクリックしてフィルターします。 [!UICONTROL オブジェクトタイプ], [!UICONTROL 操作のタイプ]，および/または [!UICONTROL ステータスタイプ].
