...
1. 任意のオブジェクト（リード、取引先責任者、アカウント、商談およびその他のカスタムエンティティ）に対して、Dynamics CRMで`new_synctomkto`という名前のカスタム 2つのオプション（ブール値）フィールドを作成します。
...
最初の同期が完了した場合でも、Marketoと同期するフィールドを選択します。
