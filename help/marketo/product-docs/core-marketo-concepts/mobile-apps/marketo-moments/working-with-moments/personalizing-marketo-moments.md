---
unique-page-id: 8782021
description: Marketo Moments をパーソナライズする方法を説明します。 モバイルアプリ表示に表示するカードとインサイトをカスタマイズします。
title: Marketo Moments のパーソナライズ
exl-id: f92c9215-0ee3-4918-8194-e6d1684a71c6
feature: Mobile Marketing
source-git-commit: 3efcb529cd3e35027f35e51dfd91f95e94af9d61
workflow-type: tm+mt
source-wordcount: '143'
ht-degree: 88%

---

# Marketo Moments のパーソナライズ {#personalizing-marketo-moments}

マーケティングプログラムやスマートキャンペーンが多数ある場合、すべてを表示できるのも素晴らしいですが、自分の作業にのみ集中したい場合があります。

>[!IMPORTANT]
>
>2023年10月2日（PT）に、アドビは Marketo モーメントアプリをすべてのアプリストアから削除しました。タブレット／モバイルデバイスにアプリが既にインストールされている場合は、その間に引き続き使用できます。Marketo Engage インスタンスが Marketo の認証の Adobe ID に移行されると、アプリにアクセスできなくなります。[詳細情報](https://nation.marketo.com/t5/product-discussions/marketo-events-app-and-marketo-moments-app-end-of-life/m-p/340712/highlight/true#M193869){target="_blank"}。

「**[!UICONTROL 自分のモーメントのみを表示]**」を有効にして、独自のメールプログラムとスマートキャンペーンのみを表示します。

![](assets/image2015-7-16-15-3a53-3a24.png)

または、「**[!UICONTROL 自分のモーメントのみを表示]**」を無効にして、アクセス権のあるすべてのスマートキャンペーンとメールプログラムを表示します。

![](assets/image2015-7-16-15-3a55-3a29.png)
