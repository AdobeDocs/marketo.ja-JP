...
description: 'Marketoのオポチュニティフィルターとトリガーの詳細。 CRMのオポチュニティデータに基づくセグメント化とトリガー:'
...
商談フィルターおよびトリガーを使うと、[!DNL Salesforce] からの商談イベントをトラックできます。 他のフィルターやトリガーとは少し違います。
...
商談フィールド（カスタムまたは標準）を探している場合は、**商談を持つ** フィルターを使用するか、**商談は`[Added/Removed/Updated]`** フィルターまたはトリガーでした。
...
条件の組み合わせに基づいて商談を持つリードを探している場合は、「**商談を持つ**、**商談に追加された**」、「商談から削除された&#x200B;**」フィルターを使用します**。 意味は以下のとおりです。
