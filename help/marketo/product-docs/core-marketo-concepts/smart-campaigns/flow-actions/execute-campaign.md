---
description: キャンペーンの実行フロー手順の使用方法を説明します。 他のユーザーがこのステップに到達したときに別のスマートキャンペーンを実行します。
title: キャンペーンの実行
...
* スケジュールキャンペーンの選定ルールが適用されます（「スケジュール」タブの「スマートキャンペーン設定」）。
...
>[!CAUTION]
>
>実行可能キャンペーンのスマートリストを無効のままにしないでください。_誰も_&#x200B;条件を満たさなくなります。 ベストプラクティスは、個別のスマートリストアセットを作成し、それらを完全に定義し、それらが有効であることを確認することです。 その後、実行可能キャンペーンで「スマートリストのメンバー」フィルターを使用して、スマートリスト定義を入れ替えることができます。
