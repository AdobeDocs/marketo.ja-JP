...
これは、Marketo Engageエンゲージメントプログラムを活用した高度育成プログラムの一例です。 ネストされたメールプログラムは、既に消費したコンテンツをユーザーが受信するのを防いだり、各ストリームで消費するコンテンツのタイプを制御したりできます。 アトリビューションレポートは、ネストされた電子メールプログラムごとに実行できます。 チャンネル：「ナーチャー」と、ネストされたメールプログラム用の専用「ナーチャー電子メール」チャンネルが、Marketo Engageの電子メールプログラムを利用して 1 通のニュースレター電子メールを送信します。 メールには、A/B テストを含めることも、含めないこともできます。
