...
これは、登録ページ、3 通の招待メール、イベントイベントプログラムを使用したフォローアップメールを含むMarketo Engageプログラムの例です。 会場でのロードショー、ランチ、ディナー、プレゼンテーションなど、登録が必要なすべてのイベントに適しています。 Marketo Engageイベントアプリを使用して、iPadでのチェックインと出席のトラッキングをおこないます。
...
>[!NOTE]
>
>イベントの前に、登録情報をiPadにダウンロードします。 次に、訪問者をチェックインし、出席情報をMarketo Engageに同期します。 タブレットをお持ちでない場合は、次の操作を実行できます。 [出席者をインポート](/help/marketo/product-docs/core-marketo-concepts/programs/working-with-programs/import-members-from-a-spreadsheet-into-a-program.md){target="_blank"} その後、プログラムの「メンバー」タブで、フォローアップメールを送信する前に実行します。
