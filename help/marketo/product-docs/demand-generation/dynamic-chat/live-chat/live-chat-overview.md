...
* エージェントインボックスはライブチャットの会話用です。 チャットがエージェントによって受け入れられていない場合、ライブチャットとして認定されないので、エージェントインボックスには表示されません。
* 「会話」タブには、ライブチャットと自動チャットの両方のすべての会話が表示されます。 したがって、会話がエージェントによって受け入れられていない場合、それは会話タブに表示されます。 リアルタイムではないので、「会話」タブには最大 24 時間の遅延があります。 ただし、エージェントインボックスはリアルタイムです。
* （エージェントまたは訪問者によって） 10 分後にチャットアクティビティがない場合、チャットはタイムアウトします。
* エージェントがライブチャット中にプロファイル画像 _account.adobe.com内）を変更した場合_ チャットが終了するまで、訪問者には古い画像が表示されます。 訪問者が次回チャットでエージェントを取得すると、新しい画像が表示されます。
