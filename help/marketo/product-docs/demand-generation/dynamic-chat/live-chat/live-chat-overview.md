...
>[!NOTE]
>
>Dynamic Chatセレクトパッケージに登録されているユーザーの場合、ライブチャットは 100 契約のライフタイムリミットを持つ体験版機能です。 この制限に達すると、ライブエージェントとのチャットをリクエストする訪問者は接続されず、代わりにグローバルなフォールバックメッセージを受け取ります。 上限を増やすには、Adobeアカウント担当者に連絡して、パッケージのアップグレードオプションについて相談してください。
