---
description: Stream Designerと、チャット機能を構築するためのカードについてご紹介します。 メッセージ、質問、ドキュメントのキャプチャ、ミーティングの予約などをダイアログに追加します。
title: ストリームデザイナー
...
<table>
 <tr>
  ...
  <td style="width:75%">応答を必要としない文を作成する場合に使用します（例：「こんにちは。 コード「SAVE25」を使用すると、今日は全品が 25% オフです）。
</td>
 </tr>
 <tr>
 ...
 <tr>
  ...
  <td>訪問者に表示されないカードはこれだけです。 特定のチャット内で、どの時点で目標を達成するかを決定するのはユーザー向けです（例：訪問者の電子メールを収集することが目標である場合は、ストリームの情報キャプチャの直後に目標カードを配置します）。</td>
 </tr>
 <tr>
 ...
 <tr>
  ...
  <li>訪問者は、ストリームでこのカードに到達するとすぐにエージェントにルーティングされるため、ライブエージェントとチャットするかどうか尋ねる質問カードを使用してこのカードの前に配置することをお勧めします。</li></td>
 </tr>
 <tr>
 ...
 <tr>
  ...
  <td>対話型フローカードを使用して、ダイアログ内のフローの複数のステップを効率化します。</td>
 </tr>
</table>

## ストリームデザイナーのアイコン {#stream-designer-icons}

ストリームデザイナーの右上に、いくつかのアイコンが表示されます。 次に、各アイコンについて説明します。
...
ダイアログまたは[対話型フォーム](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/conversational-flow-overview.md){target="_blank"}のストリームを作成できます。 この例では、ダイアログ用のストリームが作成されます。
...
1. _[!UICONTROL 質問]_&#x200B;カードをドラッグ＆ドロップします。
...
1. 「[!UICONTROL チャットボットの回答]」で、質問を入力します。
   ...
   >[!TIP]
   >
   >チャット訪問者のエクスペリエンスをカスタマイズすることもできます。
   >
   >* 「HTML を挿入」アイコン `</>` をクリックすして、対話の目的のルックアンドフィールに合わせた独自の HTML を挿入します。
   >
   >* 既知のチャット訪問者のトークンを使用して、このエクスペリエンスをパーソナライズします（例：Hello `{{lead.leadFirstName:""}}`）。 中括弧アイコン `{}` をクリックして選択します。 匿名訪問者に何か一般的なものを表示させたい場合は、引用符の間にデフォルト値を追加します（例：Hello `{{lead.leadFirstName:"there"}}`）
...
1. 「はい」の場合は、そのオプションの下にある&#x200B;_会議予約_ カードにドラッグします。
...
1. これは目標なので、ミーティング予約の下にある&#x200B;_[!UICONTROL 目標]_&#x200B;カードをドラッグします。

   ![](assets/stream-designer-12.png)

1. 目標に名前を付け（または既存のものを選択）、「**[!UICONTROL 保存]**」をクリックします。 会話スコアの割り当てはオプションです。

   ![](assets/stream-designer-13.png)

1. 「いいえ」の場合は、そのオプションの下にある別の[!UICONTROL 質問] カードにドラッグして、訪問者がメーリングリストに参加するかどうかを確認します。
...
1. 「はい」の回答の下で、_情報キャプチャ_&#x200B;カードをドラッグして、訪問者のメールを収集できます。
...
1. 「いいえ」の場合は、必ず回答を追加してください。 選択肢の 1 つは、メッセージカードをその下にドラッグして、「ありがとうございました」と表示することです。 この例では、代わりに無料のPDF ドキュメントが提供されています。

   ![](assets/stream-designer-21.png)

1. 新しいドキュメントを作成します。 名前を入力し、既にホストしている PDF の URL を入力して、「**[!UICONTROL 保存]**」をクリックします。

   ![](assets/stream-designer-22.png)

1. ダイアログをアクティブ化する準備ができたら、**[!UICONTROL 公開]**&#x200B;をクリックします。

   ![](assets/stream-designer-23.png)

>[!NOTE]
>
>[!UICONTROL 公開]をクリックする前に、[がターゲット URL](/help/marketo/product-docs/demand-generation/dynamic-chat/automated-chat/audience-criteria.md#target){target="_blank"}に入力されていることを確認してください。
