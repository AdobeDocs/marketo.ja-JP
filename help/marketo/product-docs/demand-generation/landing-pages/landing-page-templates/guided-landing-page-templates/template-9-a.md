---
unique-page-id: 8783981
description: テンプレート9-A -Marketoドキュメント — 製品ドキュメント
title: テンプレート9-A
exl-id: cc729eee-224b-4acd-87d8-7fb9155d73fb
translation-type: tm+mt
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '68'
ht-degree: 0%

---

# テンプレート9-A {#template-a}

[テンプレート9-Aのダウンロード](https://docs.marketo.com/download/attachments/8783981/template-9a.html?version=2&amp;modificationdate=1438210648000&amp;api=v2)

![](assets/image2015-7-28-15-3a9-3a26.png)

このテンプレートには、次の内容が含まれます。

* 主なセクション

   * には、ヒーローロゴ画像、ヒーローヘッダーおよびフォームが含まれます。

* 8つのボディセクション（オプション）
* フッター（オプション）

**以下をクリックして、このテンプレートをダウンロードしてください。**

[Template9-A.html](https://docs.marketo.com/download/attachments/8783981/template-9a.html?version=2&amp;modificationdate=1438210648000&amp;api=v2)
