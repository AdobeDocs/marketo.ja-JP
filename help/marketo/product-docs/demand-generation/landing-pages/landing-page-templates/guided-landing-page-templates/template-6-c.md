---
unique-page-id: 8783932
description: テンプレート6-C -Marketoドキュメント — 製品ドキュメント
title: テンプレート6-C
exl-id: f8111083-24f5-42c9-82b5-290360d72e5a
translation-type: tm+mt
source-git-commit: 72e1d29347bd5b77107da1e9c30169cb6490c432
workflow-type: tm+mt
source-wordcount: '65'
ht-degree: 0%

---

# テンプレート6-C {#template-c}

[テンプレート6-Cのダウンロード](https://docs.marketo.com/download/attachments/8783932/template-6c.html?version=1&amp;modificationdate=1437693151000&amp;api=v2)

![](assets/image2015-7-29-11-3a52-3a22.png)

このテンプレートには、次の内容が含まれます。

* 主なセクション

   * ヒーローポール、タイトル、サブタイトル、本文、ボタンが含まれます。

* 4つのボディセクション（オプション）
* フッター（オプション）

**以下をクリックして、このテンプレートをダウンロードしてください。**

[Template6-C.html](https://docs.marketo.com/download/attachments/8783932/template-6c.html?version=1&amp;modificationdate=1437693151000&amp;api=v2)
