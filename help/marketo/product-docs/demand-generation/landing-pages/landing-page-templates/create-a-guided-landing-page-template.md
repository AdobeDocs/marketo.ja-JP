...
>[!NOTE]
>
>**詳細：** 読書に飽きた？ [操作手順を説明したクールビデオ](https://youtu.be/3O7e4GdZKsM) 。
...
地域をリッチテキストとして定義すると、ユーザーは、マーケティング担当者のリッチテキストエディター [を使用して、そのコンテンツを編集できます](/help/marketo/product-docs/email-marketing/general/understanding-the-email-editor/using-the-rich-text-editor.md)。
...
編集可能な画像要素を定義するオプションは2つあります。 イメージを挿入するコンテナ `<div>`を指定するタグ、または `<img>` タグのいずれかを使用できます。
...
>[!NOTE]
>
>この `<img>` バージョンを使用する場合、レンダリングされたHTMLには、生成されたdivラッパーがタグの周りに含まれ `<img>` ます。 クラスに設定されます。」mktoImg.mktoGen,&quot;と指定し、display:inline-blockにします。
...
必須属性：**class**:&quot;mktoVideo&quot;**id**:ID文字列。 文字、数字、ダッシュ「 — 」、下線「_」のみが含まれます。 スペースは使用できません。 一意である必要があります。
**mktoName** :文字列。 これは、ランディングページエディターに表示される表示名です。 ベストプラクティスは、わかりやすい名前を使用することです。
