...
description: Marketo ランディングページをパーソナライズするためのトークンについて説明します。 トークンを使用して動的コンテンツを挿入し、エクスペリエンスをパーソナライズします。
title: トークンの概要
...
トークンを使用する場合は、デフォルト値も指定する必要があります。 このテキストは、参照しているフィールドに値がない人を示します。
...
* `{{Company.State}}`
* `{{Company.Website}}`
* カスタム会社フィールドは、表示名（`{{Company.Custom Field Name}}`など）を使用する場合にも機能します
...
[!UICONTROL マイトークン]はプログラム内で定義されています。`{{my.` で始まり、その後にトークン用に作成した名前が続きます。 [プログラム内のマイトークン](/help/marketo/product-docs/core-marketo-concepts/programs/tokens/understanding-my-tokens-in-a-program.md)を参照してください。

## メンバートークン {#member-token}

メンバートークンは、統合サービスパートナーから一意の値を挿入するために使用されます。 メンバートークンの一般的な使用方法は、ウェビナー参加者向けの一意の URL です。 各人は、`{{member.webinar url}}` トークンを使用して挿入できるウェビナーにアクセスするための一意の URL を持っています。 `{{member.webinar url}}` トークンは、サービスプロバイダーによって生成されたユーザーの一意の確認URLを自動的に解決します。
