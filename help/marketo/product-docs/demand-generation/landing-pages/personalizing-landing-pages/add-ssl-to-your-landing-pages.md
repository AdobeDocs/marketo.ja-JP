...
Marketo Engageがホストする Web フォームへの入力またはランディングページへのアクセス時、デフォルトでは、情報は保護されないプロトコル（HTTP）で送信されます。 会社のポリシーによっては、Marketo に送信される情報を（HTTPS）を使用して保護する必要が生じる場合があります。例えば、`http://info.mydomain.com/` へのアクセスは `https://info.mydomain.com/` になります。
