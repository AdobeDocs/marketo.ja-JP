---
description: インタラクティブウェビナーのユーザーおよびライセンス管理について説明します。 ウェビナーの作成者と配信者にアクセス権を割り当て、管理します。
title: ユーザーとライセンスの管理
...
<table>
  ...
  <tr>
   ...
   <td width="80%">完了したイベントと現在スケジュールされているイベント。 <a href="#things-to-note">詳細情報</a></td>
  </tr>
  <tr>
...
* イベントが作成されるたびに、それぞれのライセンスから「消費」としてカウントされます（共有ルームライセンスでない限り）。 同じ容量の「共有イベントライセンス」と「共有ルームライセンス」の両方がある場合は、「共有イベントライセンス」が優先されます。 イベントが配信されておらず、イベントプログラムがスケジュールされた時間より前に削除された場合、イベント数は補充されます。 イベントが配信されず、イベントプログラムがスケジュールされた時間より前に削除されない場合、イベントは補充されません。

* タイプ「追加ストレージライセンス」はストレージのみを提供するため、_ストレージ容量を除くすべてのフィールド_&#x200B;の値は「 – 」として表示されます。

* タイプ「共有ルームライセンス」には無制限のイベントがあり、「追加ストレージライセンス」にはストレージのみが提供されるため、これらのライセンスの「合計イベント」フィールドは「 – 」として表示されます。
...
インタラクティブウェビナーの権限を付与された、特定のインタラクティブウェビナーイベントプログラムの所有者では&#x200B;_ない_ Marketo ユーザは、それらのプログラムに対して限定的な機能を実行できます。 Marketoの管理者以外のユーザーは、プログラムにアクセスして複製できますが、インタラクティブウェビナーに対する権限を持っている場合、他の機能を実行することはできません。 ただし、Marketo 管理者ユーザは、プログラムのアクセス、変更、クローン作成、移動、削除などのすべての機能を実行できる&#x200B;_ようになります_（インタラクティブウェビナーに関する権限を持っている場合）。 Marketo 管理者および管理者以外のユーザに対してこの権限が取り消されると、それらのユーザはインタラクティブウェビナーイベントプログラムにのみアクセス可能となり、他の機能は実行できなくなります。
