---
unique-page-id: 2949777
description: LaunchPointイベントパートナー — Marketto Docs — 製品ドキュメント
title: LaunchPointイベントパートナー
translation-type: tm+mt
source-git-commit: 47b2fee7d146c3dc558d4bbb10070683f4cdfd3d
workflow-type: tm+mt
source-wordcount: '94'
ht-degree: 1%

---


# LaunchPointイベントパートナー {#launchpoint-event-partners}

MarketorをLaunchPointイベントパートナーに接続して、オンラインイベントを自動化します。 接続すると、次のことができます。

* ウェビナーイベントの管理
* Marketorランディングページでキャプチャされた登録情報を、設定済みのLaunchPointイベントパートナーにプッシュします。
* 出席情報を自動的にマーケティング担当者のイベントに取り込む
* プログラムのメンバーとしての人のステータスの取得

オンラインマーケティングイベントとLaunchPointイベントパートナーを統合する手順を参照してください。

* [WebEx](../../../../product-docs/demand-generation/events/create-an-event/create-an-event-with-webex.md)
* [Adobe Connect](../../../../product-docs/demand-generation/events/create-an-event/create-an-event-with-adobe-connect.md)
* [ON24](http://docs.marketo.com/pages/viewpage.action?pageid=2949868)
* [ReadyTalk](../../../../product-docs/demand-generation/events/create-an-event/create-an-event-with-readytalk.md)
* [GoToWebセミナー](../../../../product-docs/demand-generation/events/create-an-event/create-an-event-with-gotowebinar.md)

