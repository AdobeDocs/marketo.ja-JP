...
description: Marketoで GotoWeb セミナーを使用してイベントを作成する方法を説明します。 Marketoとの統合および同期ウェビナーの登録を行います。
...
まず、[!DNL GoToWebinar] でウェビナーを作成します。[!DNL GoToWebinar] の作成機能の設定には、Marketo で使用されるものと、[!DNL GoToWebinar] でのみ使用されるものがあります。
...
**[!UICONTROL ウェビナーのタイムゾーン]** - 該当するタイムゾーンを選択します。これは Marketo で表示できます。

**[!UICONTROL タイプ]** - **[!UICONTROL One Session]** に設定。
...
>[!TIP]
>
>追加の [!DNL GoToWebinar] ヘルプが必要な場合は、[ヘルプサイト](https://support.logmeininc.com/gotowebinar)にアクセスしてください。
...
1. これで完了です。イベントは **[!DNL GoToWebinar]** で同期され、スケジュールされました。
...
ウェビナーに新規登録したリードは、[!UICONTROL 新規ステータス]が「登録」に設定されたときに、[!UICONTROL プログラムステータスを変更]フローステップによってウェビナープロバイダーにプッシュされます。その他のステータスではプッシュされません。また、[!UICONTROL プログラムステータスを変更]フローステップ #1 と[!UICONTROL メールを送信]フローステップ #2 を必ず設定してください。
...
>[!NOTE]
>
>イベントのスケジュールを変更するには、[!DNL GoToWebinar] でウェビナーを編集する必要があります。
