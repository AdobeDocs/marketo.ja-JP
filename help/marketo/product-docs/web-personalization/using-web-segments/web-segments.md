...
* 検索した訪問者 - ドロップダウンリストから、訪問者の検索から追跡する用語を選択するか、独自の検索用語を追加します。（検索文字列を含む語句を含めるようにデフォルトで設定されているので、検索文字列に「*」（ワイルドカード）を使用する必要はありません）。

**参照元** - 訪問者が参照した URL を追加します。

* 追加する参照元を選択 - ドロップダウンリストから、追跡する参照元サイトを選択するか、独自の参照元サイトを追加します。選択すると、下のボックスに参照元が表示されます。（ワイルドカードとして「*」を使用することはできます）
...
**ページを除外する** - セグメント内で一致させたくない特定のページを除外します（ワイルドカードとして「*」を使用できます）。
...
>[!TIP]
>
>**ワイルドカードの使用 —** 何かを含む検索文字列または URL を含める場合（例：「[google.com](https://google.com)」または「検索文字列製品」）、これをワイルドカードと呼び、両端にアスタリスク（*）を付けて入力する必要があります。このように、[google.com](https://google.com) は、「*[google.com](https://google.com)*」として入力する必要があります。
