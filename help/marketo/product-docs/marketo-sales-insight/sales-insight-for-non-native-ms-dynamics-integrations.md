...
1. MS [!DNL Dynamics] セールスユーザを Marketo に同期します。
   ...
   * [営業担当者向けAPI ドキュメント](https://experienceleague.adobe.com/ja/docs/marketo-developer/marketo/rest/lead-database/sales-persons){target="_blank"}
   * [営業担当者を同期するためのAPI ドキュメント](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Sales-Persons/operation/syncSalesPersonsUsingPOST){target="_blank"}
    ...
    <tbody>
     ...
     <tr>
      ...
      <td>該当なし、これは Marketo のリード ID になります</td>
        <td>これは、同期されたMS <span class="dnl">Dynamics</span>連絡先のMarketo リード IDになります。<br><br>連絡先がMarketoで同期されたら、Marketo REST APIを使用して、大文字と小文字を区別しないMS <span class="dnl">Dynamics</span>連絡先のグローバル一意のIDをexternalPersonIdとして使用し、Marketo リードのクエリを実行できます。</td>
     </tr>
     <tr>
      <td>ロール</td>
