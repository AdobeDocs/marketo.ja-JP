...
## Marketo サンドボックスにプロビジョニングするアクションインスタンスのリクエスト {#request=an-actions-instance}
...
1. 「**Actions の設定**」を選択します。
   ...
   >[!IMPORTANT]
   >
   >メールアドレスは、サンドボックスインスタンスと本番稼働インスタンスの両方で、1 つの Actions インスタンスにのみ使用できます。 実稼動環境とサンドボックス全体で複数のインスタンスにアクセスする必要がある管理者の場合は、それぞれに異なるメールアドレスを使用する必要があります。

1. プロビジョニングカードで、セールスインサイトアクションインスタンスに招待するユーザを選択します。

## Actions インスタンスをアクティブ化 {#activate-your-actions-instance}

Actions インスタンスは、Salesforce 本番稼働アカウントでアクティブ化する必要があります。 アクティベートした後、Salesforce サンドボックスアカウントに切り替えることができます。
