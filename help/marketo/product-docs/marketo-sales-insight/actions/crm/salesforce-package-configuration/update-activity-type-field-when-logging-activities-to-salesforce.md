...
まず、タスクタイプピックリストに欠けている値を確認します（メール、電話、返信のうち）。 これを確認したり、「アクティビティのタイプ」ピックリストに変更を加えるには、Salesforce 管理者の支援が必要になる可能性があります。 これらの変更を加えるために、Salesforce 管理者は以下の手順に従うことができます。
...
これで、「タイプ」フィールドに、ログに記録されたメール、電話、返信の対応する値が表示されます。 これらの値は、セールスインサイトアクションリマインダータスクには&#x200B;_設定されません_。
