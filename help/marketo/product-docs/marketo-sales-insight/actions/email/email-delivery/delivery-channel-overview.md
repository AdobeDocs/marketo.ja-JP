---
description: Sales Insight Actionsでセールスメールを送信するための配信チャネルについて説明します。 配信チャネルでトラッキングと配信を有効にする方法を理解します。
title: 配信チャネルの概要
...
Marketo Sales サーバーは、一部のレガシー ToutApp のお客様のみ使用できます。 これらのお客様のメール設定には Marketo Sales サーバーが表示されます。 レガシー版以外のすべてのお客様には、Marketo Sales サーバーがオプションとして表示されないので、配信チャネルのロックを解除するには、Gmail や [!DNL Outlook] のアカウントを Marketo Sales に接続する必要があります。

Marketo Sales サーバーは、DKIM および SPF 認証方式をサポートしていないので、配信品質が低下する可能性があります。 このため、最高の配信品質を得るために、すべてのお客様に Gmail または [!DNL Outlook] に接続することをお勧めします。
