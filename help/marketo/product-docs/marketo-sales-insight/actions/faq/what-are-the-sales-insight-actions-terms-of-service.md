---
description: Sales Insight Actionsのサービス利用規約をご覧ください。 製品の使用に適用される法的条件を確認します。
title: セールスインサイトアクションのサービス利用規約は何ですか？
exl-id: 5a5409a0-2a26-4d5b-b1c9-89841255059d
feature: Sales Insight Actions
TQID: https://experienceleague.adobe.com/N0sMPABwPsnY-oOog8Rpq7GeIQtwoRxkzeNCTNz0WLk
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 53
ht-degree: 62%

---

# [!DNL Sales Insight Actions] のサービス利用規約は何ですか？ {#what-are-the-sales-insight-actions-terms-of-service}

[!DNL Sales Insight Actions] のサービス利用規約について詳しくは、[こちら](https://documents.marketo.com/toutapp/terms)を参照してください。

>[!NOTE]
>
>「ToutApp」は [!DNL Sales Insight Actions] の元の名前です。
