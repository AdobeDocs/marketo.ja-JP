...
1. Rest API 設定パネルの「**表示**」をクリックして、資格情報を設定します。
...
1. リモートサイト名を入力します（「MarketoSoapAPI」など）。「リモートサイト URL」に、Marketo の Soap API 設定パネルの Marketo ホスト URL を入力します。「**保存**」をクリックします。これで、Soap API 用のリモートサイト設定が作成されました。
