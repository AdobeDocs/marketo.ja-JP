...
description: Marketo Sales Insightのインサイトダッシュボードについて説明します。 エンゲージメントの速度、今後のキャンペーン、タスクをパネルで表示できます。
...
[!DNL Sales Insights] ダッシュボードで使用できる機能の詳細を説明します。
...
* ユーザは、「[!UICONTROL アカウントを表示]」アクティビティを選択できます。これにより、取引先責任者表示で、すべてのアカウントレベルの注目のアクション、メール、web アクティビティがレイヤー化されます
...
* フィルターには、「[!UICONTROL 検索]」と「[!UICONTROL 日付範囲]」が含まれます
...
* フィルターには、「[!UICONTROL 検索]」、「[!UICONTROL 招待されたイベントのみを表示]」および「[!UICONTROL 日付範囲]」が含まれます
...
* ユーザーは、特定の週をハイライト表示して、その週内のアクティビティを確認できます
* デフォルトの表示：現在の週が選択されています
* リードは取引先責任者に変換されるまで [!DNL Salesforce] のどのアカウントにも含まれないので、リードでは「[!UICONTROL アカウントアクティビティの表示]」機能を使用できません
...
>[!NOTE]
>
>アカウントまたは商談に 800 名を超える連絡先がある場合、ダッシュボードにはデータが表示されません。 ただし、個々の連絡先に移動して、インサイトとエンゲージメントを確認できます。 お使いのアカウントに 800 名を超える取引先責任者がある場合、「[!UICONTROL アカウントレベルのアクティビティの表示]」は無効になります。
