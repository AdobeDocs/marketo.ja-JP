...
description: アプリ内メッセージのテキストとスタイルを作成する方法について説明します。 メインテキストとサポートテキスト、色、サイズ、配置、トークン、タップアクションを編集できます。
...
1. **[!UICONTROL テキストのタップアクション]**&#x200B;を選択する場合は、チェックボックスをオンにします。
...
1. Apple または Android の各プラットフォームのタップアクションを選択します。
   ...
   >[!NOTE]
   >
   >タップアクションについては、Apple と Android プラットフォーム用に様々なアクションを設定できます。 例えば、ディープリンクの処理は Apple と Android で異なります。 メッセージの送信先が 1 つのプラットフォームのみの場合は、もう 1 つのプラットフォームをデフォルト設定のままにするか、「[!UICONTROL なし]」を選択します。
...
1. ドロップダウンから「マイトークン」を選択し、デフォルト値を追加して、「**[!UICONTROL 挿入]**」をクリックします。

   ![](assets/mytoken.png)

   >[!NOTE]
   >
   >オプションとして使用できるのは「マイトークン」のみです。 プログラムで「マイトークン」が設定されていない場合、[!UICONTROL トークン]ドロップダウンは空になります。
...
君は金色だ。 次に、[アプリ内メッセージボタンを設定](/help/marketo/product-docs/mobile-marketing/in-app-messages/creating-in-app-messages/set-up-the-in-app-message-button.md)します。
