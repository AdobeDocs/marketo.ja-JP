...
# 電子メールの予測コンテンツを有効にする {#enable-predictive-content-in-emails}
...
1. 予測を行う画像をクリックします。 歯車アイコンが表示されたら、その歯車アイコンをクリックし、「コンテンツを **有効にする`AI`** 」を選択します(「コンテンツ`AI` 」は、予測コンテンツの旧名です)。

   ![](assets/three.png)

1. 1つ以上のカテゴリを選択するには、「 **カテゴリ** 」ドロップダウンをクリックし、選択を行って「 **適用**」をクリックします。
...
1. 様々な画像を表示するには、「 **更新**」をクリックします。
   ...
   >[!NOTE]
   >
   >受信者が電子メールを開く **_まで、画像は選択されません_**。 プレビューに見えるのは単なる例で受信者が見るイメージとは限りません

1. 電子メールのプレビューが完了したら、 **プレビューのアクション** (Actions **)ドロップダウンをクリックし、「**&#x200B;承認して閉じる」を選択します。 まだ編集が必要な場合は、右側の「 **ドラフトを編集** 」をクリックします。
...
## Email 2.0 Editorを使用しない場合の予測コンテンツの追加 {#adding-predictive-content-when-not-using-the-email-editor}

Email 2.0 [](/help/marketo/product-docs/email-marketing/general/email-editor-2/email-editor-v2-0-overview.md) テンプレートを使用していない場合は、テンプレート内の画像をMarketorの編集可能な画像要素としてタグ付けするだけで、電子メールに予測コンテンツを追加できます。

ここでは、 [Marketor固有の構文について説明します](/help/marketo/product-docs/email-marketing/general/email-editor-2/email-template-syntax.md#elements)。
