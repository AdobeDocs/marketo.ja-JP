...
description: ユーザーを相互排他的なグループに分類するセグメントルールを定義する方法について説明します。 スマートリストのフィルターを追加し、人物（ドラフト）をプレビューして、動的コンテンツのセグメントを承認します。
title: セグメントルールの定義
...
1. フィルターに適切な値を入力します。
   ...
   >[!CAUTION]
   >
   >アカウントフィールドのアクティビティログの動作は、選定に影響を与える可能性があります。 したがって、セグメントルールを定義する際には、アカウントフィールドを使用しないことをお勧めします。
