...
詳細検索を利用して、メールを閲覧、クリックまたは返信した見込み客をターゲットにすることで、最もエンゲージメントの高い見込み客のターゲットリストを作成できます。
...
検索の日付範囲を選択します。プリセット日は、選択したメールステータス（送信済み、未配信、保留中）に応じて更新されます。

![](assets/date.png)

**ユーザー**

ユーザーセクションのメールの受信者／送信者でフィルタリングします
...
作成日、配信日、失敗日、予定日別のいずれかを選択します。使用可能なオプションは、選択したメールのステータス（送信済み、配信不能、保留中）に応じて異なります。

![](assets/when.png)

**キャンペーン**

キャンペーンへの参加でメールをフィルタリングします。
...
送信メールアクティビティ別にフィルタリングします。表示回数／表示なし、クリック数／クリックなし、返信数／返信なしを選択できます。
...
| **進行中** | 送信中のメール。メールは、数秒間以上この状態に保たれるべきではありません。 |
