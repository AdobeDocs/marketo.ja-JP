...
description: Sales Connect のSalesforce診断について説明します。 Sales Connect とSalesforce間の接続と同期の問題のトラブルシューティング。
title: Salesforce 診断
...
**トラブルシューティング手順：** 再試行ロジックでこれに対処する必要があります。 それでも問題が解決しない場合は、[!DNL Salesforce] の管理者に問い合わせて、問題のあるトリガーのトラブルシューティングを依頼します。
