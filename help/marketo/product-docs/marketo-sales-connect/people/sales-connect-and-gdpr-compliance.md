...
この場合も、ユーザーには次のようなドロップダウンオプションが提供されます。
...
「認証の種類」が更新されると、確認ポップアップが表示され、人物の詳細表示のコンプライアンスカードに更新された詳細が表示されます。
...
>[!NOTE]
>
>一度に 1 件の連絡先に対してのみ実行できます。現在、連絡先を一括エクスポートできる機能はありません。

連絡先情報を書き出すには、人物の詳細表示ヘッダーの縦並びの 3 ドットをクリックし、「**エクスポート**」をクリックします。.CSV ファイルは自動的にダウンロードされます。
...
**配信停止の同期：**&#x200B;ユーザーは、CRM（Salesforce）と配信停止を同期して、オプトアウトを最新の状態に保つことができます。\
**配信停止履歴：**&#x200B;ユーザーは、過去のオプトアウトおよびオプトインを人物の詳細表示で確認できます。\
