---
unique-page-id: 14352491
description: すべての連絡先がSalesforceからSales Connectにプッシュされなかった場合にヘルプを表示します。 同期フィルターと連絡先の条件を理解する。
title: すべてのマイ連絡先が Sales Connect にプッシュされるわけでない理由
exl-id: 53ea29c1-fe48-4808-933a-3ab1744a7d7e
feature: Marketo Sales Connect
TQID: https://experienceleague.adobe.com/OC-llK81xjzmWF2awBb3s7CtxJs-hIdh6ntueKqHea4
product_v2:
  - id: b27e5950-9033-45ac-9f86-eb22e567f615
feature_v2:
  - id: b13bd2ad-8e65-49e5-9691-2a0d31067b35
source-git-commit: a526f0bf4cbdf888b1c4462ba35dd2bc92316527
workflow-type: tm+mt
source-wordcount: 92
ht-degree: 50%

---

# [!DNL Sales Connect] にすべての取引先責任者がプッシュされない理由 {#why-didnt-all-of-my-contacts-get-pushed-to-sales-connect}

一部の連絡先のみがプッシュオーバーしている場合、会社の[!DNL Salesforce] インスタンスで「メールオプトアウト」としてマークされていない連絡先が表示される可能性があります。

グループ全体がプッシュされなかった詳しい理由については、アラートアイコンを確認してください。 会社の [!DNL Salesforce] 設定に関連する理由については、SFDC 管理者にお問い合わせください。
