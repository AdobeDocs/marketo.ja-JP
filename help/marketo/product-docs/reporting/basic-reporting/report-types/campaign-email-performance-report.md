...
description: スマートキャンペーン別にメール統計をグループ化する、Campaign メールのパフォーマンスレポートについて説明します。 開封数、クリック数、バウンス数、購読解除数をトラッキングして、キャンペーンの有効性を測定します。
...
1. 次に、レポートを調べて、キャンペーンの各メールの効果を確認します。
   ...
   | 列 | 説明 |
   |---|---|
   | [!UICONTROL ハードバウンス済み] | メールアドレスが存在しないなどの恒久的な状態が原因で、メールが拒否されました。 |
   | [!UICONTROL ソフトバウンス済み] | サーバーがダウンしている、インボックスがいっぱいになっているなどの一時的な状態が原因で、メールが拒否されました。 |
