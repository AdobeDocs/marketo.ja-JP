...
1. 矢印の 1 つを右クリック（ダブルクリックも可能）し、「**[!UICONTROL トランジションを編集]**」を選択します。
   ...
   >[!NOTE]
   >
   >「[!UICONTROL  匿名 ] ⇒ [!UICONTROL  既知 ]」のトランジションルールは編集できません。

1. 選択したトランジションについて、新しいタブが開きます。

   ![](assets/three-1.png)

1. トランジションは、リードがステージ間をどのように移動するかを制御します。右側から選択したトリガー（またはフィルター）をドラッグし、キャンバス上で放します。この例では、「**[!UICONTROL フォームへの入力]**&#x200B;トリガーを選択します。
