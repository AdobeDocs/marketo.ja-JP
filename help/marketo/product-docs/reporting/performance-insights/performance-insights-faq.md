...
description: アトリビューション設定、プログラムの可視化、機会マッピング、設定の要件など、パフォーマンスインサイトに関するよくある質問への回答を示します。 CRMと分析におけるMPIの仕組みを解説します。
...
# [!UICONTROL パフォーマンスインサイト]に関する FAQ {#performance-insights-faq}

## 「[!UICONTROL エンゲージメント]」タブの「成功」の定義は何ですか？ {#what-is-the-definition-of-success-in-the-engagement-tab}

成功は、Marketo での有意義なインタラクションの尺度です。 プログラムの目的は、リードまたは見込み客との有意義なインタラクションを作成することです。 成功は、人物がその目標に達成するステータスに達したときにマークされます。 オンラインセミナーへの参加、メールのリンククリック、web フォームの記入などです。 成功は、プログラムチャネルによって異なります。
...
1. 商談の設定
   ...
   1. プログラムセットアップ
      ...
      1. 担当者をプログラムに結び付ける
         ...
         1. ファーストタッチ属性を機能させるために、データベース内の各人物に対して新規顧客獲得プログラムと獲得日が設定されていることを確認します。
...
プログラムアナライザーを使用すると、最大 4 つの測定をまたいでプログラムを比較できます。 MPIを活用すると、成功、新規機会の創出など、選択した指標に対するチャネルやプログラムの貢献度を分析できます。また、選択した1つの特定の指標にもとづいて、12か月のチャネルのトレンドを表示することもできます。
