...
* 代わりに、次のようになります。
   ...
   * マッピングされたエージェントのミーティングカレンダーを表示します（有効な場合）。
-or-
...
<tbody>
  ...
  <tr>
    ...
    <td>xyz</td>
    <td>既存の顧客とは異なる</td>
    <td>このシナリオは、デフォルトでは新しい匿名プロファイルと見なされる新しいCookieであるかのように実行できません</td>
  </tr>
</tbody></table>

### フォントを継承するオプション {#option-to-inherit-font}

Dynamic Chatでブランドフォントを管理するのではなく、チャットボットがホストされているweb ページから直接フォントを継承できるようにしました。 このオプションを有効にすると、チャットボットはページの `<body>` タグで定義されたフォントを取得します。
...
フォームやログインなど、他のチャネルを通じて訪問者の情報を収集している場合は、その情報をDynamic Chatに直接渡すことができます。
...
ダイアログのライブチャット機能は 2023 年にリリースされましたが、対話型フローにもライブチャットのエンゲージメントを追加できるようになりました。 Marketo Engage フォームで対話型フローを使用している場合、フォームの送信直後に適格な訪問者がライブエージェントとチャットできるようにできるようになりました。
