...
# リリースノート：2020年1月 {#release-notes-jan}
...
***四半期別リリース*** 2020年1月17日にリリースされる機能は **次のとおりです**。
...
* **Microsoft Dynamics — リアルタイムでのCRMへの同期：** Marketo EngageとMicrosoft Dynamics間のリードと連絡先のリアルタイム同期。 リードまたは連絡先を作成し、「個人をMicrosoftに同期」フローアクションを使用して、すぐにMicrosoft Dynamicsでそれらを確認します。
* **LinkedInリードジェネレーションFormsの追加フィールドマッピング：** LinkedInのリードジェネレーションFormsからリードデータを取り込み、販売とマーケティングの両方のタッチポイントに対して、より関連性の高いエクスペリエンスを作成します。 非表示のフィールド、同意フィールド、およびテストリードフィールドをMarketo Engageに取り込みます。
...
>[!NOTE]
>
>* **TLS 1.0および1.1の提供終了**:Adobeのリリース構造との統合を続けるため、TLS 1.0およびTLS 1.1の廃止を2020年1月13日に移行しています。 詳しくは、こちら [を参照してください](https://nation.marketo.com/docs/DOC-7059-tls-10-11-deprecation-faq)。
   >
   >
* **ITP 2.1+ Munchkinの更新**:SafariのCookieポリシーが変更されたため、同じドメインのセッション間でユーザーを追跡するMunchkinの機能は、訪問者が使用するブラウザーとブラウザーのバージョンに基づいて、ITPによって1日または7日に制限されます。 この点を考慮して、新しいWebサービスを導入し、HTTP応答を介してMunchkin cookieをSet-Cookieヘッダに設定できるようにします。 この新しいサービスの実装方法の詳細については、 [こちらを参照してください](https://nation.marketo.com/docs/DOC-7351)。
