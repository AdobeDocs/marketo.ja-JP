...
* **登録解除の追加設定**：登録解除の追加に対する新しい設定オプションを使用して、登録解除メッセージをより詳細に制御できるようになりました。新機能により、管理者は、いつ、どこに、どのような登録解除メッセージをセールスメールに表示するかを制御できます。
...
* **ダイナミックフィールドのプロンプト**：メールの特定の部分をパーソナライズするためのプロンプトをセールスチームに表示できるようになり、さらに Sales Insight Actions のメールテンプレートに追加される新しいフィールドプロンプトを使用できます。
...
* **デフォルトのダイナミックフィールド**：デフォルトのダイナミックフィールドを使用すると、入力できる値がない場合、ダイナミックフィールドが解決できるフォールバック値を設定できます。これは、確実にすべてのパーソナライゼーションが入力された状態でメールを送信するのに役立ちます。
