...
Marketo フォームの会社名と役職を [!DNL GoToWebinar] に同期できます。 これらの追加フィールドを有効にするには、イベントパートナーに移動し、「追加フィールドを有効にする」をオンにします。
...
商談の影響アナライザーのデータを [!DNL Excel] に書き出せるようになりました。 書き出された各 [!DNL Excel] ファイルには、すべてのリード（商談で役割を持たないリードを含む）のマーケティングインタラクションと、分析で選択したアカウントのすべての商談が含まれます。 商談の行は緑色でハイライト表示されます。 特定のリードやマーケティングアクティビティに集中する必要がある場合は、[!DNL Excel] のネイティブデータフィルタリング機能を使用できます。
