...
* **メール接続スロットリング（Beta）**：[!DNL Sales Connect] 用のメール接続スロットリングを使用して、メール配信品質を向上させ、1 対 1 のセールス通信を拡張します。アドビの新しいスロットリングテクノロジーは、メールの送信タイミングを自動的に管理し、[!DNL Exchange] および Gmail ユーザに対してシームレスなエクスペリエンスを提供します。サードパーティのメール一括送信アプリケーションの使用が減少する、または不要になります。
